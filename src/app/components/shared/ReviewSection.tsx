            ...
            {reviews.map((review: any) => (
              <div key={review.id} className="border-b border-border last:border-0 pb-4 last:pb-0">
                <div className="flex items-start gap-3">
                  <img src={review.profiles?.avatar ?? `https://api.dicebear.com/7.x/avataaars/svg?seed=${review.user_id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex mb-2">
                      {[1,2,3,4,5].map((s) => (
                        <Star key={s} className={`w-3 h-3 ${review.rating >= s ? 'fill-accent text-accent' : 'text-muted-foreground'}`} />
                      ))}
                    </div>
